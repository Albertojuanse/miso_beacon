...
def calculaterssifordistance(distance, frequency=F, gain=G):
    """This function estimates the power loss of a signal that propagates certain distance and returns it as RSSI"""
    return log10(gain * pow(C / 4*pi*distance*frequency, 2))


def rangedistance(rssi, frequency=F, gain=G):
    """This function calculates the distance that a signal comes from using its RSSI value"""
    return (C / (4 * pi * frequency)) * sqrt(gain * pow(10, rssi / 10))
